...
@objc public enum StateSwiftR: Int {
    case connecting
    case connected
    case disconnected
}
...
open class SignalR: NSObject, SwiftRWebDelegate {
    ...
    open var state: StateSwiftR = .disconnected
    ...
    private func initialize() {
        ...
        let baseHTMLUrl = originUrlString.map { URL(string: $0) } ?? bundle.bundleURL
            ...
            print("wkWebView initialize in Support file")
    ...
    func processMessage(_ json: [String: Any]) {
        if let message = json["message"] as? String {
            ...
            let method = HubMethodType(rawValue: methodString ?? "")
    ...
    func runJavaScript(_ script: String, callback: ((Any?) -> ())? = nil) {
        guard wkWebView != nil else { //|| webView != nil else {
    ...
    open func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
        SceneDelegate.getAppCoordinator()?.getWindow().addSubview(wkWebView)
    }
    ...
    open func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if let id = message.body as? String {
            if wkWebView != nil {
                ...
                print("userContentController didReceive message wkWebView is nil ? ",(wkWebView == nil))
            }
        }
    }
...
open class Hub: NSObject {
    let name: String
    var handlers: [HubMethodType: [String: ([Any]?) -> ()]] = [:]
    ...
    init(name: String, connection: SignalR) {
     ...
     func onReceive(_ method: HubMethodType, callback: @escaping ([Any]?) -> ()) {
    ...
    func initialize() {
        for (method, callbacks) in handlers {
            callbacks.forEach {
                if connection != nil {
                    connection.runJavaScript("addHandler('\($0.key)', '\(name)', '\(method)')")
                }
                else {
                    printToLog("! ! SwiftR initialize() connection is nil ! !")
                }
            }
        }
    }
    ...
    open func invoke(_ method: String, arguments: [Any]? = nil, callback: ((_ result: Any?, _ error: Any?) -> ())? = nil) throws {
        guard connection != nil else {
            printToLog("SwiftR invoke error, connection = nil !")
