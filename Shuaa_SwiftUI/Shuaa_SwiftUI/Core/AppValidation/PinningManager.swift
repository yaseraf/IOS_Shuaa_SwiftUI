...
func publicKeyHash(from certificatePath: String) -> String? {
    guard let certData = try? Data(contentsOf: URL(fileURLWithPath: certificatePath)) else {
        return nil
    }
    ...
    guard let secTrust = trust, let publicKey = SecTrustCopyKey(secTrust) else {
        return nil
    }
    ...
    var fullKey = Data(asn1Header)
    fullKey.append(publicKeyData)

    // SHA256 hash
    var digest = [UInt8](repeating: 0, count: Int(CC_SHA256_DIGEST_LENGTH))
    fullKey.withUnsafeBytes {
        _ = CC_SHA256($0.baseAddress, CC_LONG(fullKey.count), &digest)
    }
...
//// Example
//if let hash = publicKeyHash(from: "/path/to/certificate.cer") {
