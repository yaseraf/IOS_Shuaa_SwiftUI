...
struct SelectBankContentView: View {
    ...
    var banksData:Binding<[GetBankIBANUIModel]?>
//    var banksData:Binding<[BankUIModel]?>
    var selectedBank:Binding<GetBankIBANUIModel?>
    
    var onConfirm:(GetBankIBANUIModel?) -> Void
    var onDismiss:() -> Void
    ...
    private var contentView: some View {
        VStack {
            ForEach(Array((banksData.wrappedValue ?? []).enumerated()), id: \.offset) { index, item in
                HStack {
                    Text("\(item.bankName ?? "") / \(String(item.bankAccNo?.suffix(5) ?? ""))")
                        .font(.apply(.semiBold, size: 14))
                    
                    Spacer()
                    
                    Image(selectedBank.wrappedValue?.bankAccNo == item.bankAccNo ?? "" ? "ic_checkmarkBlue" : "ic_checkmarkEmpty")
                        .resizable()
                        .scaledToFit()
                        .frame(width: 24, height: 24)
                }
