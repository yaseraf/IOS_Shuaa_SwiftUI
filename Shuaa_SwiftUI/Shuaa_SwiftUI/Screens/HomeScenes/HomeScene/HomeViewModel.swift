...
class HomeViewModel: ObservableObject {
    ...
    private var lookupsUseCase: LookupsUseCaseProtocol
    ...
    @Published var connectionHubAPIResult:APIResultType<[GetCompaniesLookupsUIModel]>?
...
extension HomeViewModel {
    ...
    func openFilterScene() {
        coordinator.openFilterScene(delegate: self, categoryFilter: categoryFilter, specificSharesFilter: specificShareFilter ?? .none, sectorTypeFilter: sectorTypeFilter ?? .none)
    }
    ...
    func openShareDetails(selectedStock: GetMarketWatchByProfileIDUIModel) {
        ...
        coordinator.openShareDetailsScene()
    }
}
...
extension HomeViewModel {
        ...
        Task.init {
            await useCase.getUserAccounts(requestModel: requestModel) {[weak self] result in
                ...
                case .success(let success):
                    ...
                    if KeyChainController().clientID != nil && KeyChainController().clientID != "" && KeyChainController().mainClientID != nil && KeyChainController().mainClientID != "" && UserDefaultController().selectedUserAccount?.AccType != "all" {
                    }
                    ...
                    if UserDefaultController().selectedUserAccount == nil ||  UserDefaultController().selectedUserAccount?.ClientID == "" || UserDefaultController().selectedUserAccount?.AccType == "all" {
                        for item in success {
                            if item.IS_DEFAULT_ACC == "Y" {
                                ...
                                KeyChainController().clientID = item.ClientID
                                KeyChainController().mainClientID = item.MainClientID
                                KeyChainController().accountID = item.AccountID
                                KeyChainController().nin = item.NIN
                            }
                        }
                    }
    ...
    func GetCompaniesLookupsAPI(success:Bool, searchSymbol:String) {
        
        UserDefaultController().searchResultSymbol = searchSymbol
...
extension HomeViewModel {
    ...
    func UnSubscribleMarketWatchSymbolsSignalR(){
        ...
        if Connection_Hub.shared.chatHub != nil {
            do {
                debugPrint("test invoke unSubscribeMarketWatchSymbols '\(arraySymbols)'")
                ...
                try Connection_Hub.shared.chatHub?.invoke(HubMethodType.unSubscribeMarketWatchSymbols.rawValue, arguments: [KeyChainController().username ?? ""]) { (result, error) in
                    if let e = error {
                        debugPrint("unSubscribeMarketWatchSymbols invoke '\(arraySymbols)' Error: \(e)")
                    } else {
                        debugPrint("unSubscribeMarketWatchSymbols invoke '\(arraySymbols)' Success!, appDelegate.userNameNotEncryptrd\("info3@fitmena.com")")
                        self.SubscribeMarketWatchSymbolsSignalR()
                    }
                }
            } catch let error {
                debugPrint("unSubscribeMarketWatchSymbols chatHub '\(arraySymbols)' error: \(error.localizedDescription)")
            }
        }
    }
}
...
extension HomeViewModel: WatchlistFilterDelegate {
...
extension HomeViewModel: MarketWatchDelegate {
