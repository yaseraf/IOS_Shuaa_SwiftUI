...
struct HomeContentView: View {
        ...
        HStack {
            ...
            if showHeader {
                ...
                VStack(alignment: .center, spacing: 0) {
                    ...
                    HStack {
                        ...
                        Text("\(AppUtility.shared.formatThousandSeparator2Decimal(number: Double(exchangeSummaryData.wrappedValue?.first?.netChangePerc ?? "") ?? 0))%")
                            .font(.apply(.semiBold, size: 14))
                    }
                }
            ...
            Button {
                ...
                Circle()
                    ...
                    .frame(width: 24, height: 24)
                    .overlay(
                        Image(showHeader ? "ic_upArrow" : "ic_downArrow")
                            .resizable()
                            .scaledToFit()
                            .frame(width: 12, height: 12)
                    )
        ...
        VStack(spacing: 0) {
            ...
            ScrollView(.vertical, showsIndicators: false) {
                if UserDefaultController().isClassificationBySectorEnabled == true {
                    sectorsView(list: marketWatchData)
                } else {
                    sectorlessView
                }
            }
    ...
    private var groupedMarketWatch: [String: [GetMarketWatchByProfileIDUIModel]] {
        ...
        return Dictionary(grouping: sourceData) { item in
            let sector = AppUtility.shared.isRTL ? item.sectorA?.trimmingCharacters(in: .whitespacesAndNewlines) : item.sectorE?.trimmingCharacters(in: .whitespacesAndNewlines)
    ...
    func sectorsView(list: Binding<[GetMarketWatchByProfileIDUIModel]?>) -> some View {
        ...
        return ForEach(sortedKeys, id: \.self) { sectorCode in
            VStack(spacing: 0) {
                ...
                HStack {
                    Text(
                        AppUtility.shared.isRTL ? (groupedMarketWatch[sectorCode]?.first?.sectorA?.isEmpty == false ? groupedMarketWatch[sectorCode]?.first?.sectorA : "others".localized) ?? "others".localized : (sectorCode.isEmpty ? "others".localized : sectorCode)
                    )
...
//                debugPrint("did not flash: no newValue \(newValue), for \(updatedSymbol) and \(item.symbol ?? "")")
//                return
//            }
//            if newValue == (item.symbol ?? "") {
//                debugPrint("did flash: \(newValue), for \(updatedSymbol) and \(item.symbol ?? "")")
//                flash()
